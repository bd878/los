...
compt2ulong(comp_t comptime) /* convert compt_ to unsigned long */
{
  unsigned long val;
  int exp;

  val = comptime & 0x1fff; /* 13 bits is mantissa */
  exp = (comptime >> 13) & 7; /* 3 bits exponenta (0-7) */
  while (exp-- > 0)
    val *= 8;
  return(val);
}
...
main(int argc, char *argv[])
{
  ...
  while (fread(&acdata, sizeof(acdata), 1, fp) == 1) {
    /* fread - read binary stream of 1 acct structure */
    /* from *fp to acdata */
